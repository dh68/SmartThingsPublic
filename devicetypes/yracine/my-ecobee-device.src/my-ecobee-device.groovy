...
metadata {
	...
	definition(name: "My Ecobee Device", author: "Yves Racine",  namespace: "yracine") {
		...
		attribute "coolingSetpointRangeHigh", "number"
		attribute "coolingSetpointRangeLow", "number"
		...
		attribute "programCoolTemp", "number"
		attribute "programHeatTemp", "number"
		attribute "programCoolTempDisplay", "number"
		attribute "programHeatTempDisplay", "number"
		...
		attribute "weatherTemperatureDisplay", "number"
		...
		attribute "weatherTempHigh", "number"
		attribute "weatherTempLow", "number"
		attribute "weatherTempHighDisplay", "number"
		attribute "weatherTempLowDisplay", "number"
...
void coolLevelUp() {
	...
	try  {   
		coolingSetpointRangeHigh=device.latestValue("coolingSetpointRangeHigh")
	} catch (any) {
		traceEvent(settings.logFilter,"coolLevelUp>not able to get coolingSetpointRangeHigh ($coolingSetpointRangeHigh), using default value",
...
void coolLevelDown() {
	...
	try  {   
		coolingSetpointRangeLow=device.latestValue("coolingSetpointRangeLow")
	} catch (any) {
		traceEvent(settings.logFilter,"coolLevelDown>not able to get coolingSetpointRangeLow ($coolingSetpointRangeLow), using default value",
			true, get_LOG_INFO(), true)        
	}    
...
void heatLevelUp() {
	...
	try  {   
		...
		traceEvent(settings.logFilter,"heatLevelUp>not able to get heatingSetpointRangeHigh ($heatingSetpointRangeHigh), using default value",
			true, get_LOG_INFO(), true)        
	}    
    
	heatingSetpointRangeHigh= (heatingSetpointRangeHigh) ?: (scale == 'C')?30:99  
	if (scale == 'C') {
		double nextLevel = device.currentValue("heatingSetpoint").toDouble() 
		nextLevel = (nextLevel + 0.5).round(1)        
		if (nextLevel > heatingSetpointRangeHigh.toDouble().round(0)) {
			nextLevel = heatingSetpointRangeHigh.toDouble().round(0)
		}
...
void heatLevelDown() {
	...
	heatingSetpointRangeLow= (heatingSetpointRangeLow) ?: (scale == 'C')?10:50       
	if (scale == 'C') {
		double nextLevel = device.currentValue("heatingSetpoint").toDouble() 
		nextLevel = (nextLevel - 0.5).round(1)        
		if (nextLevel < heatingSetpointRangeLow.toDouble().round(0)) {
			nextLevel = heatingSetpointRangeLow.toDouble().round(0)
		}
		setHeatingSetpoint(nextLevel)
	} else {
		int nextLevel = device.currentValue("heatingSetpoint")
...
void setHeatingSetpoint(temp) {
	...
	try  {   
		...
		traceEvent(settings.logFilter,"setHeatingSetpoint>not able to get heatingSetpointRangeLow ($heatingSetpointRangeLow) or heatingSetpointRangeHigh ($heatingSetpointRangeHigh), using default values",
			true, get_LOG_INFO(), true)        
	}    
	heatingSetpointRangeLow= (heatingSetpointRangeLow)?: (scale == 'C')?10:50       
	heatingSetpointRangeHigh= (heatingSetpointRangeHigh)?: (scale == 'C')?30:99  
...
void setCoolingSetpoint(temp) {
	...
	try  {   
		...
		traceEvent(settings.logFilter,"setCoolingSetpoint>not able to get coolingSetpointRangeLow ($coolingSetpointRangeLow) or coolingSetpointRangeHigh ($coolingSetpointRangeHigh), using default values",
			true, get_LOG_INFO(), true)        
	}    
	coolingSetpointRangeLow= (coolingSetpointRangeLow) ?: (scale == 'C')?10:50       
	coolingSetpointRangeHigh= (coolingSetpointRangeHigh) ?:  (scale == 'C')?30:99  
...
private def refresh_thermostat(thermostatId, asyncValues=null, RTEventFlag=false) {
	...
	def heatingSetpointRangeHigh= (data.thermostatList[0]?.settings?.heatRangeHigh) ?: (scale=='C')?30:99
	def heatingSetpointRangeLow= (data.thermostatList[0]?.settings?.heatRangeLow) ?: (scale=='C')?10:50
	def coolingSetpointRangeHigh= (data.thermostatList[0]?.settings?.coolRangeHigh) ?: (scale=='C')?30:99
	def coolingSetpointRangeLow= (data.thermostatList[0]?.settings?.coolRangeLow) ?: (scale=='C')?10:50
	def dataEvents = [
